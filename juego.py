...
while ejecutando:
    ...
    if colision_rojo:
        puntuacion += 100
    if not enemigo_negro and not enemigo_amarillo and not enemigo_azul and not enemigo_rojo:
        enemigoamarillo = EnemigoAmarillo()
        enemigo_amarillo.add(enemigoamarillo)
        enemigonegro = EnemigoNegro()
        enemigo_negro.add(enemigonegro)
    ...
    muestra_texto(PANTALLA, consolas, str("Ingrse nombre jugador: "), BLANCO, 60,500,300)
    barra_hp(PANTALLA, 750, 15, jugador.hp)
pygame.quit()

print()
print('Puntaje obtenido: '+str(puntuacion))
nombre = input('Ingrese nombre para guardar su puntaje: ')

def insertar_puntaje():
    os.environ['TNS_ADMIN'] = '/Users/julit/Proyectos_Python/Wallet_BDPRI2021'
    connection = cx_Oracle.connect('PABLOREINOSO', '.Inacap2021.', 'bdpri2021_high')
    cursor = connection.cursor()
    sql = "INSERT INTO juego(nombre_usuario, puntaje) VALUES('{}','{}')".format(nombre, puntuacion)
    cursor.execute(sql)
    connection.commit()
    print()
    print('!LOS 10 MEJORES PUNTAJES¡')
    for result in cursor.execute("SELECT * FROM juego order by puntaje desc FETCH FIRST 10 ROWS ONLY"):
        print(result)

print(insertar_puntaje())
